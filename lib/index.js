...
/**
 * Read one session's tokenCost projection: the live fold while the session is
 * still resident, otherwise its persisted projection-cache row. Returns null
 * when neither carries a value yet (a brand-new session, or one this profile
 * never folded) — the browser half treats that as "not available yet".
 * @param ctx - plugin context (services resolved lazily, each optional).
 * @param sessionId - the session to read.
 * @returns the projection value, or null.
 */
async function sessionTokenCost(ctx, sessionId) {
	...
	if (sessions !== void 0 && typeof sessions.get === "function") {
		try {
			...
			if (session !== void 0 && session !== null) {
				const block = projections?.snapshot(session);
				if (block !== void 0 && block.values.tokenCost !== void 0) return block.values.tokenCost;
			}
		} catch {
			// fall through to the persisted row
		}
	}
	const persistence = ctx.get("sessionPersistence");
	const cache = ctx.get("sessionProjectionCache");
	if (persistence === void 0 || cache === void 0) return null;
	try {
		...
		webCtx.effect(() => webCtx.webServer.register({
			...
			handler: async (req, res) => {
				...
				try {
					...
					if (pathname === "/api/token-cost/session") {
						const sessionId = url.searchParams.get("id");
						res.writeHead(200, {
							"content-type": "application/json; charset=utf-8",
							"cache-control": "no-store"
						});
						res.end(JSON.stringify({ ok: true, sessionId, value: await sessionTokenCost(webCtx, sessionId) }));
						return;
					}
