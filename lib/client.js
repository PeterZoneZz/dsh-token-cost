...
`;
		...
		const en = {
			...
			"widget.close": "Close",
			"widget.error": "Widget render error: {message}",
		...
		/** How often the focused session's host-side projection is re-read. */
		...
		/** Finite-number coercion: anything else folds to 0 so formatting never throws. */
		...
		/** Sum of the four disjoint buckets; tolerates a partial or missing token bag. */
		function totalTokensOf(tokens) {
			if (tokens === null || typeof tokens !== "object") return 0;
			return numOf(tokens.uncachedInput) + numOf(tokens.cacheRead) + numOf(tokens.cacheWrite) + numOf(tokens.output);
		}
		/** One bucket's count from a token bag. */
		function bucketOf(tokens, key) {
			return tokens === null || typeof tokens !== "object" ? 0 : numOf(tokens[key]);
		}
		...
		/** Poll one JSON endpoint (a null path stays idle); null until the first response lands. */
		function useJsonEndpoint(path, pollMs = 6e4) {
			const [state, setState] = (0, react.useState)(null);
			(0, react.useEffect)(() => {
				// Never let a previous key's payload stand in for the new one.
				setState(null);
		...
		/**
		 * Best-effort synchronous read of one session's projection value from
		 * the client store. The surface moved between host releases — the early
		 * web build exposed `projectionValues`, rc.6 exposes
		 * `projections.faceOf(key)` (the shape the built-in
		 * `dsh-client-ui-permission-presets` and `-goal` plugins read) — so both
		 * are probed and any failure degrades to null. A missing read must never
		 * take the widget down.
		 */
		function projectionOf(session, key) {
			if (session === null || typeof session !== "object") return null;
			try {
				const viaFace = session.projections?.faceOf?.(key);
				const value = viaFace?.getSnapshot?.();
				if (value !== void 0 && value !== null) return value;
			} catch {
				// fall through to the legacy shape
			}
			try {
				const legacy = session.projectionValues?.[key];
				return legacy === void 0 ? null : legacy;
			} catch {
				return null;
			}
		}
		/**
		 * Normalize an endpoint payload. The routes answer in two shapes: the
		 * balance route wraps its body as `{ok, value}`, while the session and
		 * summary routes send the value flat. Accept both, and turn a negative
		 * envelope into null.
		 */
		function payloadOf(reply) {
			if (reply === null || typeof reply !== "object") return null;
			if (reply.ok === false) return null;
			if (reply.ok === true && reply.value !== void 0) return reply.value;
			return reply;
		}
		//#endregion
		//#region card pieces
		...
		function HeroSection({ balance, t, initialBalance }) {
			...
			if (balance.ok && balance.value !== void 0) {
				...
				if (info === void 0) return (0, react_jsx_runtime.jsxs)("div", {
					className: styles.hero,
					children: [(0, react_jsx_runtime.jsx)("div", {
						className: styles.heroMain,
						children: [(0, react_jsx_runtime.jsx)("div", { className: styles.heroLabel, children: t("balance.title") }), (0, react_jsx_runtime.jsx)("div", { className: styles.heroSub, children: t("balance.unavailable") })]
					})]
				});
				...
				return (0, react_jsx_runtime.jsxs)("div", {
					...
					children: [(0, react_jsx_runtime.jsxs)("div", {
						...
						}, current), sub.length > 0 ? (0, react_jsx_runtime.jsx)("div", { className: styles.heroSub, children: sub.join(" · ") }) : null]
					})]
				});
			}
		...
		function SessionSection({ proj, t }) {
			// `proj` is null (not undefined) whenever the projection has no value
			// yet, so the guard must cover both — reading `.tokens` off null used
			// to throw during the first render of the opened card and take the
			// whole widget down with it.
			if (proj === null || proj === void 0 || typeof proj !== "object" || totalTokensOf(proj.tokens) <= 0) {
				return (0, react_jsx_runtime.jsxs)("div", {
			...
			const shown = buckets.filter((bucket) => bucketOf(proj.tokens, bucket.key) > 0);
			return (0, react_jsx_runtime.jsxs)("div", {
				className: styles.section,
					...
					children: [(0, react_jsx_runtime.jsxs)("div", {
						...
						children: shown.map((bucket) => (0, react_jsx_runtime.jsxs)("span", {
							...
							children: [(0, react_jsx_runtime.jsx)("span", { className: `${styles.swatch} ${bucket.cls}` }), (0, react_jsx_runtime.jsx)("span", { children: bucket.label }), (0, react_jsx_runtime.jsx)("span", {
								...
								children: formatTokens(bucketOf(proj.tokens, bucket.key))
							})]
						}, bucket.key))
					})]
				})]
		...
		function TotalSection({ summary, t }) {
			// `/api/token-cost/summary` answers with the aggregate itself (flat),
			// not the `{ok, value}` envelope the balance route uses; reading
			// `summary.ok` here left this section permanently "unavailable".
			const body = payloadOf(summary);
			if (body === null || body.cost === void 0) {
				return (0, react_jsx_runtime.jsxs)("div", {
					className: styles.section,
			...
			const sym = currencyOf(body.currency);
			const byModel = Object.entries(body.byModel ?? {});
			return (0, react_jsx_runtime.jsxs)("div", {
				className: styles.section,
				children: [(0, react_jsx_runtime.jsx)("div", { className: styles.sectionLabel, children: t("total.title") }), (0, react_jsx_runtime.jsxs)("div", {
					...
					children: [(0, react_jsx_runtime.jsxs)("div", {
						...
						children: [(0, react_jsx_runtime.jsx)("span", {
							className: styles.rowValue,
							children: `${sym}${formatCost(body.cost)}`
						}), (0, react_jsx_runtime.jsx)("span", {
							className: styles.sub,
							children: t("total.sessions", { n: numOf(body.sessions) })
						})]
					}), byModel.length > 0 ? (0, react_jsx_runtime.jsx)("div", {
						className: styles.models,
						children: byModel.map(([model, entry]) => `${shortModel(model)} ${formatTokens(totalTokensOf(entry?.tokens))} ${sym}${formatCost(entry?.cost)}`).join(" · ")
					}) : null]
				})]
			});
		}
		...
		/**
		 * Render-error fence. React unmounts the entire root when a component
		 * throws during render, which is how one bad card field used to take the
		 * whale itself with it. The fence keeps the widget mounted and degrades
		 * only the crashed region to a one-line message.
		 */
		class WidgetBoundary extends react.Component {
			...
			componentDidCatch(error) {
				try {
					console.error("[dsh-token-cost] render error contained by the widget boundary", error);
				} catch {
					// no console: the fallback still renders
				}
			}
			render() {
		...
		/** Human-readable text for a caught render error. */
		...
		function TokenCostWidget({ t, sessionsList }) {
			...
			// The current session comes from the sessions list store — the same
			// `current` / `byId` pair the app shell reads for the window title.
			...
			const sessionsSnapshot = (0, react.useSyncExternalStore)(subscribeSessions, readSessions);
			const currentSessionId = sessionsSnapshot?.current;
			const currentSession = currentSessionId === void 0 || sessionsSnapshot?.byId === void 0 ? null : (sessionsSnapshot.byId[currentSessionId] ?? null);
			// Primary read: the host's own fold for exactly this session, which is
			// the value the sidebar column shows. It replaced a client-side
			// `projectionValues` lookup that the rc.6 client store does not have
			// (it would have been null forever); the store read stays as a
			// zero-latency secondary source.
			const sessionReply = useJsonEndpoint(
				typeof currentSessionId === "string" && currentSessionId.length > 0 ? `/api/token-cost/session?id=${encodeURIComponent(currentSessionId)}` : null,
				SESSION_POLL_MS
			);
			const hostTokenCost = payloadOf(sessionReply);
			const currentTokenCost = hostTokenCost !== null && hostTokenCost !== void 0 ? hostTokenCost : projectionOf(currentSession, "tokenCost");
			...
			/** Fade the whale only until some channel answers; a degraded answer is still an answer. */
			(0, react.useEffect)(() => {
				setDim(balance === null && summary === null && currentTokenCost === null);
			...
			return (0, react_jsx_runtime.jsx)("div", {
				...
				children: [open && (0, react_jsx_runtime.jsx)("div", {
					...
					}, "header"), (0, react_jsx_runtime.jsx)(WidgetBoundary, {
						fallback: (error) => (0, react_jsx_runtime.jsxs)("div", {
							className: styles.section,
							children: [(0, react_jsx_runtime.jsx)("div", { className: styles.sectionLabel, children: t("widget.title") }), (0, react_jsx_runtime.jsx)("div", { className: `${styles.sectionBody} ${styles.degraded}`, children: t("widget.error", { message: errorTextOf(error) }) })]
						}),
						children: [(0, react_jsx_runtime.jsx)(HeroSection, { balance, t, initialBalance }, "hero"), (0, react_jsx_runtime.jsx)(SessionSection, { proj: currentTokenCost, t }, "session"), (0, react_jsx_runtime.jsx)(TotalSection, { summary, t }, "total"), (0, react_jsx_runtime.jsx)("div", { className: styles.note, children: t("note") }, "note")]
					}, "body")]
				}, "panel"), (0, react_jsx_runtime.jsx)("span", {
		...
		function apply(ctx) {
			...
			ctx.effect(() => {
				...
				// The outer fence guarantees something stays on screen even if the
				// widget itself fails to render: the whale never vanishes silently.
				root.render((0, react_jsx_runtime.jsx)(WidgetBoundary, {
					...
					children: (0, react_jsx_runtime.jsx)(TokenCostWidget, { t, sessionsList })
				}));
				return () => {
					root.unmount();
					host.remove();
		...
		exports.WidgetBoundary = WidgetBoundary;
		exports.apply = apply;
